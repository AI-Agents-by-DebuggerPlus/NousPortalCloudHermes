...
- **Подтверждено логами:** cloud `wss://…/v1/ws/chat` отвечает **403** на handshake с `Authorization: Bearer` (`keyLen=40`, ключ Inference). Раньше этот шлюз принимал cookie → `POST /api/auth/ws-ticket` → `?ticket=`, а не `sk-nous`. Пока 403 не снят, Send голоса невозможен.
- Дальше: авторизация WS (ticket/cookie или иной способ, который шлюз реально принимает), затем проверка `attachments[]` и STT tool.
- Большой base64 в одном текстовом WS-кадре может упереться в лимит размера кадра агента.
- Серверный STT tool — вне этого репозитория.
- `HermesHttpSseClient` можно удалить отдельным коммитом, когда desktop тоже уйдёт с Inference.
